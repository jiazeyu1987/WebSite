...
const categories: Category[] = [
  { id: "home", name: "首页", icon: <HomeIcon className="w-6 h-6" /> },
  { id: "cardiac", name: "心内介入类", icon: <CardiacIcon className="w-6 h-6" /> },
  { id: "neuro", name: "神经介入类", icon: <NeuroIcon className="w-6 h-6" /> },
  { id: "peripheral", name: "外周介入类", icon: <PeripheralIcon className="w-6 h-6" /> },
  { id: "ortho", name: "骨科介入类", icon: <OrthoIcon className="w-6 h-6" /> },
  { id: "urology", name: "泌尿介入类", icon: <UrologyIcon className="w-6 h-6" /> },
  { id: "nonvascular", name: "非血管介入类及其他", icon: <NonVascularIcon className="w-6 h-6" /> },
  { id: "prefab", name: "医疗预制件", icon: <PrefabIcon className="w-6 h-6" /> },
]
...
export function CategoryNav({ activeCategory, onCategoryChange }: CategoryNavProps) {
  return (
    <nav className="flex items-center justify-start gap-2 px-4 py-3 bg-card border-b border-border overflow-x-auto">
      {categories.map((category) => (
        <button
          key={category.id}
          onClick={() => onCategoryChange(category.id)}
          className={cn(
            "flex flex-col items-center justify-center min-w-[70px] md:min-w-[90px] px-3 py-2 rounded-lg transition-all",
            "hover:bg-secondary",
        ...
        >
          ...
          <span className="text-xs md:text-sm font-medium whitespace-nowrap">{category.name}</span>
        </button>
      ))}
    </nav>
  )
